...
def build_main_keyboard():
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("⚡ رشق قنوات ومجموعات", callback_data="service_spam"),
        ],
        [
            InlineKeyboardButton("📱 أرقام وهمية", callback_data="service_numbers"),
        ],
        [
        ...
        [
            InlineKeyboardButton(f"🛒 شراء رصيد {DEVELOPER_USERNAME}", callback_data="buy_credits"),
        ],
        [
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        "🌟 مرحباً بك في بوت الخدمات التجارية!\n\n"
        f"👤 {user.mention_html()}\n"
        f"💰 رصيدك الحالي: <b>{balance}</b> نقطة\n\n"
        "📌 الخدمات المتاحة:\n"
        f"⚡ <b>رشق القنوات والمجموعات</b> - التكلفة: {SPAM_SERVICE_COST} نقطة لكل طلب\n"
        f"📱 <b>أرقام وهمية مؤقتة</b> - التكلفة: {NUMBERS_SERVICE_COST} نقطة لكل رقم\n\n"
        "✅ اختر الخدمة من القائمة أدناه للبدء:"
    )
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    data = query.data
    user_id = query.from_user.id
    if data.startswith(("num_", "picknum_", "fetch_sms", "spam_execute")):
        return  # هذه تُعالج داخل المحادثات
    ...
    if data == "my_balance":
        ...
        create_user(user.id, user.username)
        balance = get_balance(user.id)
        text = f"🏠 <b>القائمة الرئيسية</b>\n💰 رصيدك: <b>{balance}</b> نقطة\n\n"
        await query.edit_message_text(
